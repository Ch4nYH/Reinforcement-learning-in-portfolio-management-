...
class PG:
    def __init__(self,M,L,N,name,load_weights,trainable,type,number):
        ...
        if trainable == 'True':
            # Initial summary
            self.summary_writer = tf.summary.FileWriter('./result/PG/', self.session.graph)
    ...
    def train(self):
        ...
        profit, _ = self.session.run([self.profit, self.optimize], 
                                     feed_dict={self.state:s,self.out:np.reshape(a,(-1,self.M)),self.future_price:np.reshape(p,(-1,self.M)),self.w_previous:np.reshape(a_previous,(-1,self.M))})
